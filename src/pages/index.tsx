...
import { Shield, Lock, Sliders, Globe, Info, Menu, DoorOpen, Link2, Key, FileLock, FileText, QrCode, Gauge } from "lucide-react";
...
function PanelContent({
  ...
  return (
    <div className="h-full relative">
      ...
      <div className={`h-full ${active === "password-generator" ? "" : "hidden"}`}>
        <PasswordGenerator />
      </div>
      <div className={`h-full ${active === "file-vault" ? "" : "hidden"}`}>
        <FileVault />
      </div>
      <div className={`h-full ${active === "secure-notes" ? "" : "hidden"}`}>
        <SecureNotes />
      </div>
      <div className={`h-full ${active === "qr-crypto" ? "" : "hidden"}`}>
        <QrCrypto />
      </div>
      <div className={`h-full ${active === "entropy" ? "" : "hidden"}`}>
        <EntropyChecker />
      </div>
    </div>
  );
